...
extraLookAndFeel:: extraLookAndFeel() : LookAndFeel_V4()
{
    dialImg = ImageCache::getFromMemory(BinaryData::xtraDial_png, BinaryData::xtraDial_pngSize);
}

void extraLookAndFeel::drawRotarySlider(Graphics& g, int x, int y, int width, int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, Slider& slider){
    if(dialImg.isValid()){
        // normalize
        ...
        int imgWidth = dialImg.getWidth();
        int imgHeight = dialImg.getHeight() / frames;
        g.drawImage(dialImg, 0, 0, imgWidth, imgHeight, 0, frameId * imgHeight, imgWidth, imgHeight);
    }
    else
    {
...
void extraLookAndFeel::drawLabel(Graphics& g, Label& label)
{
